...
type ArrowIcon = 'up' | 'down' | 'left' | 'accept';

// Typed by icon name so a missing waveform glyph fails the type check.
const WAVE_ICONS: Record<Exclude<IconName, ArrowIcon>, string> = {
  ...
  vmsBurst: `<path ${stroke} d="M1 9 L4 9 L4 3 L6 3 L6 15 L8 15 L8 3 L10 3 L10 15 L12 15 L12 3 L14 3 L14 15 L16 15 L16 9 L23 9"/>`,
  vmsFr: `<path ${stroke} d="M1 5 L23 5 M1 13 L23 13 M4 2 L4 8 M6 2 L6 8 M8 2 L8 8 M4 10 L4 16 M6 10 L6 16 M8 10 L8 16 M20 3 L20 7 M20 11 L20 15"/>`,
...
const ARROW_ICONS: Record<ArrowIcon, string> = {
...
const isArrow = (name: IconName): name is ArrowIcon => name in ARROW_ICONS;
...
export function statusIconSvg(name: IconName): string {
  return `<svg class="icon icon-status" viewBox="0 0 24 18" aria-hidden="true">${STATUS_ICONS[name] ?? (isArrow(name) ? ARROW_ICONS[name] : WAVE_ICONS[name])}</svg>`;
}

// Electrode lead shown with the pad contact quality bars.
export const PAD_GLYPH = `<svg class="icon pad-glyph" viewBox="0 0 16 12" aria-hidden="true"><path ${stroke} d="M1 11 C4 11,4 5,7 5 L10 5 M10 3 L10 7 M10 3 L15 3 M10 7 L15 7"/></svg>`;
